...
vector<double> Matrix::jacobi(vector<double> b, double epsilon, int iterMax) {
    Matrix* aux = new Matrix();
    *aux = *this;
    int n = this->getRows();
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i != j) {
                this->m[i][j] = this->m[i][j] / this->m[i][i];
            }
        }
        b[i] = b[i] / this->m[i][i];
        x[i] = b[i];
    }
    for (int k = 1; k < iterMax + 1; k++) {
        for (int i = 0; i < n; i++) {
            double s = 0;
            for (int j = 0; j < n; j++) {
                if (i != j) {
...
vector<double> Matrix::seidel(vector<double> b, double epsilon, int iterMax) {
    Matrix* aux = new Matrix();
    *aux = *this;
    int n = this->getRows();
    vector<double> x(n, 0.0);
